import {HSVtoRGB, RGBtoHSV, HSLtoRGB, RGBtoHSL} from '../colorUtils';
...
describe('Color Utils', () => {
    describe('HSVtoRGB', () => {
        ...
        it('should handle hue values > 360 by wrapping', () => {
            const result1 = HSVtoRGB(360, 100, 100);
            const result2 = HSVtoRGB(0, 100, 100);
            expect(result1).toEqual(result2);
            expect(result1).toEqual([255, 0, 0]);
        });

        it('should handle hue values < 0 by wrapping', () => {
            const result1 = HSVtoRGB(-60, 100, 100);
            const result2 = HSVtoRGB(300, 100, 100);
            expect(result1).toEqual(result2);
        });

        it('should clamp saturation values > 100', () => {
            const result = HSVtoRGB(0, 150, 100);
            expect(result).toEqual([255, 0, 0]);
        });
        ...
        it('should clamp value/brightness values > 100', () => {
            const result = HSVtoRGB(0, 100, 150);
            expect(result).toEqual([255, 0, 0]);
        });
    ...
    describe('RGBtoHSV', () => {
        ...
        it('should clamp RGB values > 255', () => {
            const result = RGBtoHSV(300, 300, 300);
            expect(result).toEqual([0, 0, 100]);
        });

        it('should clamp RGB values < 0', () => {
            const result = RGBtoHSV(-50, -50, -50);
            expect(result).toEqual([0, 0, 0]);
        });
    ...
    describe('HSLtoRGB', () => {
        ...
        it('should handle hue values > 360 by wrapping', () => {
            const result1 = HSLtoRGB(360, 100, 50);
            const result2 = HSLtoRGB(0, 100, 50);
            expect(result1).toEqual(result2);
        });

        it('should clamp saturation and lightness values', () => {
            expect(HSLtoRGB(0, 150, 50)).toEqual([255, 0, 0]);
            expect(HSLtoRGB(0, -50, 50)).toEqual([128, 128, 128]);
            expect(HSLtoRGB(0, 100, 150)).toEqual([255, 255, 255]);
            expect(HSLtoRGB(0, 100, -50)).toEqual([0, 0, 0]);
        });
    ...
    describe('RGBtoHSL', () => {
        ...
        it('should clamp RGB values > 255', () => {
            const result = RGBtoHSL(300, 300, 300);
            expect(result).toEqual([0, 0, 100]);
        });
    ...
    describe('Edge cases and boundary conditions', () => {
        ...
        it('should handle all maximum values', () => {
            expect(HSVtoRGB(359, 100, 100)).toEqual([255, 0, 4]);
            expect(RGBtoHSV(255, 255, 255)).toEqual([0, 0, 100]);
            expect(HSLtoRGB(359, 100, 50)).toEqual([255, 0, 4]);
            expect(RGBtoHSL(255, 255, 255)).toEqual([0, 0, 100]);
        });

        it('should handle very small values', () => {
            const rgb1 = HSVtoRGB(0.1, 0.1, 0.1);
            expect(rgb1[0]).toBeGreaterThanOrEqual(0);
            expect(rgb1[0]).toBeLessThanOrEqual(255);

            const hsv1 = RGBtoHSV(1, 1, 1);
        ...
        it('should handle decimal input values correctly', () => {
            const result = HSVtoRGB(180.5, 50.5, 75.5);
