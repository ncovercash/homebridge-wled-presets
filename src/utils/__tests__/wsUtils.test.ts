import {WLEDWebSocket, loadEffectsViaHTTP, type WLEDResponse} from '../wsUtils';
...
describe('WLEDWebSocket', () => {
    ...
    beforeEach(() => {
        ...
        wsClient = new WLEDWebSocket('192.168.1.100');
    });
    ...
    describe('Constructor', () => {
        it('should create a WLEDWebSocket instance with the correct host', () => {
            const client = new WLEDWebSocket('192.168.1.100');
            expect(client).toBeInstanceOf(WLEDWebSocket);
        });
    });
    ...
    describe('connect', () => {
        it('should connect to the correct WebSocket URL', async () => {
            ...
            expect(WebSocket).toHaveBeenCalledWith('ws://192.168.1.100/ws');
        });
    ...
    describe('message handling', () => {
        it('should call onStateUpdate callback when receiving state message', async () => {
            ...
            const testMessage: WLEDResponse = {
    ...
    describe('reconnection', () => {
        ...
        it('should stop reconnecting after max attempts', async () => {
            ...
            expect(WebSocket).toHaveBeenCalledTimes(11); // Initial + 10 reconnection attempts
        });
    });
...
describe('loadEffectsViaHTTP', () => {
    ...
    it('should load effects from WLED device', async () => {
        ...
        const result = await loadEffectsViaHTTP('192.168.1.100');
    ...
    it('should throw error on HTTP failure', async () => {
        ...
        await expect(loadEffectsViaHTTP('192.168.1.100')).rejects.toThrow('Error while loading effects on 192.168.1.100: Network error');
    });

    it('should handle non-Error rejections', async () => {
        mockedAxios.get.mockRejectedValue('String error');

        await expect(loadEffectsViaHTTP('192.168.1.100')).rejects.toThrow('Error while loading effects on 192.168.1.100: String error');
    });
});
